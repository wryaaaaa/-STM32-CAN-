...
/*引脚配置: PB6=SCL, PB7=SDA (避免与CAN的PB8/PB9冲突) */
...
void OLED_ShowFloatNum(uint8_t Line, uint8_t Column, float Number, uint8_t IntegerLength, uint8_t DecimalLength)
{
    ...
    OLED_ShowNum(Line, Column, temp, IntegerLength);
    ...
    for (i = 0; i < DecimalLength; i++)
    {
        Number *= 10;
        temp = (uint32_t)Number;
        OLED_ShowChar(Line, Column + IntegerLength + 1 + i, temp + '0');
        Number -= temp;
    }
}
